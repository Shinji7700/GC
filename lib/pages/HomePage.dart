...
import 'package:flutter_app/Tiles/group_tile.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  _getUserAuthAndJoinedGroups() async {
    ...
    DatabaseService(uid: _user.email).getUserGroups().then((snapshots) {
      // print(snapshots);
      setState(() {
        _groups = snapshots;
      });
  ...
  void _popupDialog(BuildContext context) {
    ...
    Widget createButton = FlatButton(
      ...
      onPressed: () async {
        if (_groupName != null) {
          await Data.getUserNameSharedPreference().then((val) {
            DatabaseService(uid: _user.email).createGroup(val, _groupName);
          });
          Navigator.of(context).pop();
        }
      },
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer: Drawer(
        child: ListView(
          children: <Widget>[
            Center(child: Icon(Icons.account_circle, size: 100.0, color: Colors.grey[700])),
